...
import { ScrollView, StyleSheet, Touchable, TouchableWithoutFeedback, View } from 'react-native';
import { Avatar, Button, Checkbox, Chip, Dialog, List, Text, TouchableRipple, useTheme } from 'react-native-paper';
...
export default function BasketItem({ item, setIsLoading }) {
    ...
    const deleteRecept = async () => {
        const basketArray = await getStorage('@basket');
        const newBasketArray = basketArray.filter((prevItem) => prevItem.title !== selected);
        await saveInStorage('@basket', newBasketArray);
        closeDialog();
    }
