...
export default function RecipeMinDrawer({ recipes, handleCrow }) {
    const [openDetailViews, setOpenDetailViews] = React.useState(Array(recipes.description.recipeArray.length).fill(false));
    const theme = useTheme();

    const panResponders = React.useMemo(() =>
        recipes.description.recipeArray.map((item, index) =>
            PanResponder.create({
        ...
        <View style={{ height: '100%' }}>
            {recipes.description.recipeArray.map((item, index) => (
                ...
                >
                    ...
                    >
                        <Text style={{ fontSize: 18 }}>{recipes.description.preparation[item.title]}</Text>
                    </ScrollView>
